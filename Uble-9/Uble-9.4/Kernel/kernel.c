/* ========================================================================
 * UBLE GRAPHICAL OPERATING SYSTEM - SPEED & UTILITY PERFORMANCE CORE
...
typedef struct {
    ...
    char label[32];
    uint32_t action_id; 
    uint32_t linked_data; // Targets item selections directly
} DesktopWidget;
...
/* Draws a compact 14x14 system layout gear icon - FIXED COMPILER PARAMETER HERE */
void draw_gear_icon(uint32_t x, uint32_t y, uint32_t color) {
    draw_rect(x + 5, y + 2, 4, 10, color);
    draw_rect(x + 2, y + 5, 10, 4, color);
    draw_rect(x + 3, y + 3, 8, 8, color);
    draw_rect(x + 5, y + 5, 4, 4, PAL_DARK_GRAY);
}

/* Helper to convert integer to clean raw string mapping */
...
void draw_custom_window(int x, int y, int w, int h, const char* title, uint32_t close_act_id) {
    ...
    if (num_widgets < 45) {
        ...
        widgets[num_widgets].linked_data = 0;
        num_widgets++;
    }
}
...
void mouse_wait(uint8_t type) {
    uint32_t timeout = 2000; // Drastically optimized hardware poll loops
    while (timeout--) {
...
#define ATA_TIMEOUT 400 // Reduced timeout values to speed up file performance
...
void check_hard_drive_presence(void) {
    ...
    if (status == 0xFF || status == 0x00) { has_hard_drive = false; } 
    else { has_hard_drive = true; }
}
...
bool fs_save_to_disk(void) {
    ...
    if (sectors > 40) sectors = 40; // Hard speed clamp on volatile updates
...
void process_command(const char* cmdline) {
    ...
    if (strcmp(cmd, "help") == 0) {
        ...
        if (has_hard_drive) print_string("Storage State: Virtual Hard Disk Mounted.\n");
    } else if (strcmp(cmd, "files") == 0) {
        print_string("Stored System Files:\n");
        for (int i = 0; i < MAX_FILES; i++) {
            if (files[i].used) { print_string(" - "); print_string(files[i].name); print_string("\n"); }
        }
    } else {
...
void draw_file_manager(void) {
    ...
    draw_string("File Name", EXPLORER_X + 12, EXPLORER_Y + 30, PAL_BLACK);
    draw_string("Size", EXPLORER_X + 170, EXPLORER_Y + 30, PAL_BLACK);
    draw_string("Type", EXPLORER_X + 260, EXPLORER_Y + 30, PAL_BLACK);
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        if (files[i].used) {
            // Draw interactive row clickable zone link
            draw_rect(EXPLORER_X + 8, print_y - 2, EXPLORER_WIDTH - 16, 14, PAL_LIGHT_GRAY);
            draw_string(files[i].name, EXPLORER_X + 12, print_y, PAL_BLUE);
            
            char sz_buf[12];
            int_to_str(files[i].size, sz_buf);
            draw_string(sz_buf, EXPLORER_X + 170, print_y, PAL_BLACK);
            draw_string("TXT File", EXPLORER_X + 260, print_y, PAL_DARK_GRAY);
            ...
            if (num_widgets < 45) {
                ...
                strcpy(widgets[num_widgets].label, "OPEN_ROW");
                widgets[num_widgets].action_id = 200;
                widgets[num_widgets].linked_data = i; // Save exact index tracking
                num_widgets++;
            }
...
void draw_settings_panel(void) {
    ...
    draw_string("Uble OS Storage & Memory Layout", SETTINGS_X + 15, SETTINGS_Y + 35, PAL_BLACK);
    draw_rect(SETTINGS_X + 12, SETTINGS_Y + 48, SETTINGS_WIDTH - 24, 1, PAL_DARK_GRAY);

    draw_string("Active Alloc Memory: 32 MB Base RAM", SETTINGS_X + 15, SETTINGS_Y + 60, PAL_BLACK);

    int file_count = 0;
    for (int i = 0; i < MAX_FILES; i++) { if (files[i].used) file_count++; }
    char f_buf[16]; int_to_str(file_count, f_buf);
    draw_string("Total Files Cached: ", SETTINGS_X + 15, SETTINGS_Y + 85, PAL_BLACK);
    draw_string(f_buf, SETTINGS_X + 180, SETTINGS_Y + 85, PAL_BLUE);

    draw_string("Disk Space Used   : 40 Sectors", SETTINGS_X + 15, SETTINGS_Y + 110, PAL_BLACK);
    draw_string("Disk Space Free   : 16344 Sectors", SETTINGS_X + 15, SETTINGS_Y + 135, PAL_GREEN);
}

void draw_start_menu(void) {
    int mx = 5; int my = 28; int mw = 150; int mh = 80;
    draw_rect(mx, my, mw, mh, PAL_LIGHT_GRAY);
    draw_rect(mx, my, mw, 2, PAL_WHITE); draw_rect(mx, my, 2, mh, PAL_WHITE);
    draw_rect(mx + mw - 2, my, 2, mh, PAL_DARK_GRAY); draw_rect(mx, my + mh - 2, mw, 2, PAL_DARK_GRAY);
    ...
    if (num_widgets < 45) {
        ...
        widgets[num_widgets].action_id = 101; widgets[num_widgets].linked_data = 0; num_widgets++;
        
        widgets[num_widgets].x = mx; widgets[num_widgets].y = my + 27;
        widgets[num_widgets].w = mw; widgets[num_widgets].h = 22;
        strcpy(widgets[num_widgets].label, "LAUNCH_NOTE");
        widgets[num_widgets].action_id = 102; widgets[num_widgets].linked_data = 0; num_widgets++;

        widgets[num_widgets].x = mx; widgets[num_widgets].y = my + 49;
        widgets[num_widgets].w = mw; widgets[num_widgets].h = 22;
        strcpy(widgets[num_widgets].label, "LAUNCH_CLEAR");
        widgets[num_widgets].action_id = 103; widgets[num_widgets].linked_data = 0; num_widgets++;
    }
}

/* Draws direct text file shortcut grid spaces cleanly onto the wallpaper background */
void draw_desktop_shortcuts(void) {
    int start_dx = 30;
    int start_dy = 60;
    int count = 0;
    for (int i = 0; i < MAX_FILES; i++) {
        if (files[i].used && count < 3) { // Constrain to avoid overlaps
            draw_rect(start_dx, start_dy, 40, 32, PAL_LIGHT_GRAY);
            draw_rect(start_dx + 4, start_dy + 4, 32, 2, PAL_DARK_GRAY);
            draw_rect(start_dx + 4, start_dy + 10, 24, 2, PAL_DARK_GRAY);
            draw_rect(start_dx + 4, start_dy + 16, 28, 2, PAL_DARK_GRAY);
            
            draw_string(files[i].name, start_dx, start_dy + 36, PAL_WHITE);

            if (num_widgets < 45) {
                widgets[num_widgets].x = start_dx; widgets[num_widgets].y = start_dy;
                widgets[num_widgets].w = 50; widgets[num_widgets].h = 48;
                strcpy(widgets[num_widgets].label, "SHORTCUT");
                widgets[num_widgets].action_id = 200; // Directly reuse layout loaders
...
void desktop_redraw_pipeline(void) {
    ...
    // Render direct wallpaper text file shortcuts
    ...
    if (num_widgets < 45) {
        ...
        widgets[num_widgets].action_id = 99; widgets[num_widgets].linked_data = 0; num_widgets++;
    }

    // Right-aligned settings gear placement
    int gx = fb_width - 30;
    draw_gear_icon(gx, 6, PAL_WHITE);
    if (num_widgets < 45) {
        widgets[num_widgets].x = gx - 2; widgets[num_widgets].y = 4;
        widgets[num_widgets].w = 18; widgets[num_widgets].h = 18;
        strcpy(widgets[num_widgets].label, "SET_GEAR");
        widgets[num_widgets].action_id = 300; widgets[num_widgets].linked_data = 0; num_widgets++;
    }

    draw_string("Uble Graphical Core System", 160, 8, PAL_WHITE);
    ...
    for (int i = 0; i < num_widgets; i++) {
        if (x >= widgets[i].x && x <= (widgets[i].x + widgets[i].w) &&
            ...
            if (widgets[i].action_id == 99) {
                start_menu_open = !start_menu_open; desktop_redraw_pipeline(); return;
            }
            if (widgets[i].action_id == 300) {
                settings_open = !settings_open; start_menu_open = false; desktop_redraw_pipeline(); return;
            }
            if (widgets[i].action_id == 44) {
                draw_rect(TERM_X + 5, TERM_Y + 25, TERM_WIDTH - 10, TERM_HEIGHT - 30, PAL_BLACK);
                term_cursor_x = TERM_X + 5; term_cursor_y = TERM_Y + 25; return;
            }
            if (widgets[i].action_id == 55) {
                explorer_open = false; desktop_redraw_pipeline(); return;
            }
            if (widgets[i].action_id == 66) {
                notepad_open = false; desktop_redraw_pipeline(); return;
            }
            if (widgets[i].action_id == 88) {
                settings_open = false; desktop_redraw_pipeline(); return;
            }
            if (widgets[i].action_id == 101) {
                explorer_open = true; start_menu_open = false; desktop_redraw_pipeline(); return;
            }
            if (widgets[i].action_id == 102) {
                notepad_open = true; start_menu_open = false; desktop_redraw_pipeline(); return;
            }
            ...
            if (widgets[i].action_id == 200) {
                // One-click structural loader: Pulls data array instantly back to Notepad
                int target_file_idx = widgets[i].linked_data;
                strcpy(notepad_buffer, files[target_file_idx].data);
            ...
            if (widgets[i].action_id == 77) {
                ...
                if (n_idx != -1) {
                    ...
                    else { strcpy(files[n_idx].data, "Empty notepad text."); }
                    files[n_idx].size = strlen(files[n_idx].data);
                    fs_save_to_disk();
                    notepad_saved_count++;
                    
                    term_cursor_x = TERM_X + 5;
                    print_string("Fast Save Completed: "); print_string(filename_buf); print_string("\n");
                }
...
void kernel_main(MultibootInfo* mbi) {
    ...
    // Fast-boot screen init
    desktop_redraw_pipeline();
    save_mouse_backbuffer();

    check_hard_drive_presence();
    mouse_init();
    ...
    if (fs_find("readme.txt") == -1) {
        ...
        if(idx != -1) {
            strcpy(files[idx].data, "Welcome to your fast performance Uble environment!");
            files[idx].size = strlen(files[idx].data);
        }
    }
    ...
    int cmd_idx = 0; main_cmd_buf[0] = '\0';
    ...
    while (1) {
        ...
        for (volatile int delay = 0; delay < 10000; delay++); // Lower optimized iteration delay
    }
}
