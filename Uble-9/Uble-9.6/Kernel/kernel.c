/* ========================================================================
 * UBLE GRAPHICAL OPERATING SYSTEM - TOTAL PERFORMANCE DESKTOP KERNEL
...
void draw_file_manager(void) {
    ...
    draw_string("Type", EXPLORER_X + 260, EXPLORER_Y + 26, PAL_BLACK);
    ...
    for (int i = 0; i < MAX_FILES; i++) {
        if (files[i].used) {
            if (strncmp(files[i].name, current_working_dir, strlen(current_working_dir)) == 0) {
                ...
                draw_string(files[i].is_dir ? "Folder" : "Text", EXPLORER_X + 260, print_y, PAL_DARK_GRAY);
    ...
    draw_rect(bx, by, 80, 24, PAL_LIGHT_GRAY); draw_string("New Dir", bx + 12, by + 6, PAL_BLACK);
    if (num_widgets < 80) {
        widgets[num_widgets].x = bx; widgets[num_widgets].y = by; widgets[num_widgets].w = 80; widgets[num_widgets].h = 24;
        strcpy(widgets[num_widgets].label, "TRIGGER_MKDIR"); widgets[num_widgets].action_id = 501; num_widgets++;
    }
    draw_rect(bx + 85, by, 75, 24, PAL_LIGHT_GRAY); draw_string("Rename", bx + 12, by + 6, PAL_BLACK);
    if (num_widgets < 80) {
    ...
    draw_rect(bx + 165, by, 75, 24, PAL_LIGHT_GRAY); draw_string("Delete", bx + 12, by + 6, PAL_BLACK);
    if (num_widgets < 80) {
        widgets[num_widgets].x = bx + 165; widgets[num_widgets].y = by; widgets[num_widgets].w = 75; widgets[num_widgets].h = 24;
        strcpy(widgets[num_widgets].label, "TRIGGER_DELETE"); widgets[num_widgets].action_id = 503; num_widgets++;
    }
    draw_rect(bx + 245, by, 50, 24, PAL_LIGHT_GRAY); draw_string("..", bx + 18, by + 6, PAL_BLACK);
    if (num_widgets < 80) {
        widgets[num_widgets].x = bx + 245; widgets[num_widgets].y = by; widgets[num_widgets].w = 50; widgets[num_widgets].h = 24;
...
void desktop_redraw_pipeline(void) {
    ...
    draw_string("Start", 16, 8, PAL_WHITE);
    if (num_widgets < 80) {
        widgets[num_widgets].x = 5; widgets[num_widgets].y = 2; widgets[num_widgets].w = 60; widgets[num_widgets].h = 24;
