...
#define ACTION_COPY_FILE   505
#define ACTION_CUT_FILE    506
#define ACTION_PASTE_FILE  507
...
void draw_mouse_cursor_fist(int x, int y)
{
    /* outline */
    draw_rect(x+4,  y+1, 8, 1, PAL_BLACK);

    draw_rect(x+3,  y+2, 1, 8, PAL_BLACK);
    draw_rect(x+12, y+2, 1, 9, PAL_BLACK);

    draw_rect(x+4,  y+11, 8, 1, PAL_BLACK);

    /* knuckles */
    draw_rect(x+4, y+2, 2, 3, PAL_WHITE);
    draw_rect(x+6, y+2, 2, 3, PAL_WHITE);
    draw_rect(x+8, y+2, 2, 3, PAL_WHITE);
    draw_rect(x+10,y+2, 2, 3, PAL_WHITE);

    /* knuckle shadows */
    draw_rect(x+4, y+5, 2, 1, PAL_LIGHT_GRAY);
    draw_rect(x+6, y+5, 2, 1, PAL_LIGHT_GRAY);
    draw_rect(x+8, y+5, 2, 1, PAL_LIGHT_GRAY);
    draw_rect(x+10,y+5, 2, 1, PAL_LIGHT_GRAY);

    /* palm */
    draw_rect(x+4, y+6, 8, 5, PAL_WHITE);

    /* thumb wrapped across fist */
    draw_rect(x+1, y+6, 4, 3, PAL_LIGHT_GRAY);

    /* palm shadows */
    draw_rect(x+4, y+9, 8, 2, PAL_LIGHT_GRAY);

    /* deep shadow area */
    draw_rect(x+8, y+7, 4, 3, PAL_DARK_GRAY);

    /* finger separation lines */
    draw_rect(x+6, y+2, 1, 4, PAL_DARK_GRAY);
    draw_rect(x+8, y+2, 1, 4, PAL_DARK_GRAY);
    draw_rect(x+10,y+2, 1, 4, PAL_DARK_GRAY);

    /* thumb outline */
    draw_rect(x+1, y+6, 1, 3, PAL_BLACK);

    /* lower contour */
    draw_rect(x+3, y+10, 1, 1, PAL_BLACK);
    draw_rect(x+11,y+10, 1, 1, PAL_BLACK);

    /* highlight */
    draw_rect(x+5, y+7, 2, 1, PAL_WHITE);
}
...
static void draw_button(int x, int y, int w, int h, const char* text, uint32_t bg)
{
    // shadow (gives depth)
    draw_rect(x + 1, y + 1, w, h, PAL_DARK_GRAY);

    // main button
    draw_rect(x, y, w, h, bg);

    // top highlight edge
    draw_rect(x, y, w, 1, PAL_WHITE);
    draw_rect(x, y, 1, h, PAL_WHITE);
    ...
    // centered-ish text
    draw_string(text, x + 8, y + 6, PAL_BLACK);
}
...
void draw_file_manager(void)
{
    ...
    // Leave room for two rows of controls
    draw_rect(
        explorer_x + 4,
        explorer_y + 22,
        EXPLORER_WIDTH - 8,
    ...
    draw_string("Name", explorer_x + 12, explorer_y + 26, PAL_BLACK);
    ...
    for (int i = 0; i < MAX_FILES; i++)
    {
        ...
        const char* sub_item =
            files[i].name + strlen(current_working_dir);
        ...
        if (num_widgets < 80)
        {
            widgets[num_widgets].x =
                explorer_x + 6;

            widgets[num_widgets].y =
                print_y - 2;

            widgets[num_widgets].w =
                EXPLORER_WIDTH - 12;

            widgets[num_widgets].h = 14;

            strcpy(
                widgets[num_widgets].label,
                "SELECT_ROW"
            );
    ...
    if (num_widgets < 80)
    {
        ...
        widgets[num_widgets].y =
            toolbar_y + 2;
        widgets[num_widgets].w = 60;
        widgets[num_widgets].h = 20;
    ...
    int bx = explorer_x + 6;
    int by = explorer_y + EXPLORER_HEIGHT - 30;
...
{
    int top = get_window_at_point(mx, my);
    if (top == 0)
        return false;

    if (top == WIN_TERM &&
        terminal_open &&
        point_in_rect(mx, my, term_x, term_y, TERM_WIDTH, 22))
    {
    ...
    if (top == WIN_SETTINGS &&
        settings_open &&
        point_in_rect(mx, my, settings_x, settings_y, SETTINGS_WIDTH, 22))
    {
    ...
    if (top == WIN_NOTEPAD &&
        notepad_open &&
        point_in_rect(mx, my, notepad_x, notepad_y, NOTEPAD_WIDTH, 22))
    {
    ...
    if (top == WIN_EXPLORER &&
        explorer_open &&
        point_in_rect(mx, my, explorer_x, explorer_y, EXPLORER_WIDTH, 22))
    {
...
void desktop_evaluate_click(int x, int y)
{
    ...
    for (int i = num_widgets - 1; i >= 0; i--)
    {
        ...
        else if (aid == 55 || aid == 210 || aid == 501 || aid == 502 || aid == 503 || aid == 504 || aid == ACTION_COPY_FILE || aid == ACTION_CUT_FILE || aid == ACTION_PASTE_FILE) 
        {
        ...
        else if (aid == 900)
        {
            ...
            if (selected_idx < 0 || selected_idx >= MAX_FILES)
                return;

            if (!files[selected_idx].used)
                return;

            if (files[selected_idx].is_dir)
            {
                strcpy(current_working_dir, files[selected_idx].name);
                explorer_open = true;
                active_window = WIN_EXPLORER;
                bring_window_to_front(WIN_EXPLORER);
                selected_file_idx = -1;
            }
            else
            {
                strcpy(notepad_buffer, files[selected_idx].data);
                notepad_buf_idx = strlen(notepad_buffer);
                notepad_open = true;
                active_window = WIN_NOTEPAD;
                bring_window_to_front(WIN_NOTEPAD);
            }
            desktop_redraw_pipeline();
            return;
        ...
        else if (aid == ACTION_COPY_FILE)
        {
            if (selected_file_idx != -1 && files[selected_file_idx].used)
            {
                clipboard_file_idx = selected_file_idx;
                clipboard_is_cut = false;
            }
        }
        else if (aid == ACTION_CUT_FILE)
        {
            if (selected_file_idx != -1 && files[selected_file_idx].used)
            {
                clipboard_file_idx = selected_file_idx;
        ...
        else if (aid == ACTION_PASTE_FILE)
        {
            if (clipboard_file_idx == -1)
                return;

            if (!files[clipboard_file_idx].used)
                return;

            // build destination path
            char base_name[128];
            strcpy(base_name, files[clipboard_file_idx].name);
            ...
            char new_path[128];
            strcpy(new_path, current_working_dir);
            strcat(new_path, filename);

            // prevent self-paste overwrite
            if (fs_find(new_path) != -1)
                return;

            // CREATE COPY
            int new_idx = fs_create(new_path, files[clipboard_file_idx].is_dir);
            if (new_idx == -1)
                return;

            if (!files[clipboard_file_idx].is_dir)
            {
                strcpy(files[new_idx].data, files[clipboard_file_idx].data);
                files[new_idx].size = files[clipboard_file_idx].size;
            }

            fs_save_to_disk();

            // CUT = delete original after successful paste
            if (clipboard_is_cut)
            {
                fs_delete_node(clipboard_file_idx);
                clipboard_file_idx = -1;
                clipboard_is_cut = false;
            }
        }
    ...
    if (!handled)
    {
        int top = get_top_window_at_point(x, y);
        ...
        // LOCK: only top window is allowed to interpret ANY click here
        // even if no widget exists at this position
        ...
        switch (top)
        {
            ...
            case WIN_SETTINGS:
            case WIN_EXPLORER:
                begin_window_drag(x, y); // only triggers if titlebar matches inside function
                return;
        }
    }
...
void kernel_main(MultibootInfo* mbi) {
    ...
    while (1) {
        ...
        if (mouse_x != last_mouse_x || mouse_y != last_mouse_y) {
            ...
            if (dragging_window)
                draw_mouse_cursor_fist(mouse_x, mouse_y);
            else
