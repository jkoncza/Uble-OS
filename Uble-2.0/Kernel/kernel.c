...
void ata_write_sectors(uint32_t lba, uint8_t count, uint16_t* buf) {
    outb(0x1F6, 0xE0 | ((lba >> 24) & 0x0F));
    outb(0x1F2, count);
    outb(0x1F3, (uint8_t)lba);
    outb(0x1F4, (uint8_t)(lba >> 8));
...
void fs_save_to_disk(void) {
    ...
    static uint16_t buffer[(sizeof(files) + 1) / 2];
    uint32_t i;
    ...
    outb(0x1F7, 0xE7);      // ATA FLUSH CACHE
    ata_wait(0x80, 0x00);  // wait for BSY to clear (with timeout)
