...
/* ==== FIXED: Increased buffer to hold a full file payload cleanly ==== */
#define MAX_CMD_LENGTH  4120
#define MAX_USERS       10
...
#define FS_MAX_SECTORS     2048  // Allows up to 1 MB of storage
...
/* ==== Custom Uble Executable (.eu) structures ==== */
#pragma pack(push, 1)
typedef struct {
    uint8_t magic[4];     // 'U', 'B', 'L', 'E'
    uint16_t version;     // Version of Uble interpreter
    uint16_t program_len; // Length of the bytecode payload
} UbleHeader;
#pragma pack(pop)
...
#define OP_PRINT_STR    0x03  // Prints inline null-terminated string payload
#define OP_LOAD_REG     0x04  // Load immediate value into Register: [Op][Reg][Val]
#define OP_ADD          0x05  // RegA = RegA + RegB: [Op][RegA][RegB]
#define OP_SUB          0x06  // RegA = RegA - RegB: [Op][RegA][RegB]
#define OP_PRINT_REG    0x07  // Prints decimal value stored in a Register: [Op][Reg]
#define OP_JMP          0x08  // Unconditional Jump to a PC offset: [Op][Offset_High][Offset_Low]
#define OP_JZ           0x09  // Jump if Register is Zero: [Op][Reg][Offset_High][Offset_Low]
#define OP_JNZ          0x0A  // Jump if Register is Not Zero: [Op][Reg][Offset_High][Offset_Low]
#define OP_CMP          0x0B  // Compares RegA and RegB, sets Zero Flag: [Op][RegA][RegB]
#define OP_INC          0x0C  // Increment register by 1: [Op][Reg]
#define OP_DEC          0x0D  // Decrement register by 1: [Op][Reg]
#define OP_SET_COLOR    0x0E  // Updates screen print attribute color: [Op][ColorByte]
#define OP_READ_REG     0x0F  // Waits for a key press (single digit '0'-'9'), saves numeric value to Reg: [Op][Reg]
#define OP_MUL          0x10  // RegA = RegA * RegB: [Op][RegA][RegB]
#define OP_DIV          0x11  // RegA = RegA / RegB: [Op][RegA][RegB]
#define OP_MOD          0x12  // RegA = RegA % RegB: [Op][RegA][RegB]
...
/* ==== FIXED: Multi-line backspace calculation wrap-around handles rows safely ==== */
...
bool fs_save_to_disk(void) {
    ...
    if (sectors > FS_MAX_SECTORS) {
        print_string("FS too large for disk allocation\n");
        return false;
    }
    ...
    for (uint32_t s = 0; s < sectors; s++) {
        ...
        if (!ata_write_sectors(FS_LBA_START + s, 1, (uint16_t*)sector_buf)) {
            print_string("FS sector write failed\n");
            return false;
        }
    }
...
bool fs_load_from_disk(void) {
    ...
    if (header->magic != FS_MAGIC || header->version != 1 || 
...
/* ==== Hex Parser helper to interpret escape syntax loops ==== */
static uint32_t parse_hex_escapes(char* dest, const char* src, uint32_t max_len) {
    uint32_t d_idx = 0;
    uint32_t s_idx = 0;

    while (src[s_idx] != '\0' && d_idx < max_len) {
        if (src[s_idx] == '\\' && src[s_idx + 1] == 'x') {
            // Found hex token signature
            char h1 = src[s_idx + 2];
            char h2 = src[s_idx + 3];
...
void cmd_help(void) {
    ...
    print_string("  write <name> <text>    - Write file (interprets \\xNN symbols)\n");
    print_string("  rm <name>               - Delete file\n");
    print_string("  run <name>              - Run custom Uble Binary (.eu)\n");
}
...
void cmd_write(const char* name, const char* text) {
    ...
    // Translate the user text inputs containing \xXX into raw custom binary format
    parse_hex_escapes(files[idx].data, text, MAX_FILE_DATA);
...
/* ==== FULLY UPDATED UBLE INTERPRETER ENGINE ==== */
void cmd_run(const char* name) {
    ...
    File* f = &files[idx];
    if (sizeof(UbleHeader) > MAX_FILE_DATA) return;

    UbleHeader* header = (UbleHeader*)f->data;

    // Check UBLE .eu Custom Header
    if (header->magic[0] != 'U' || header->magic[1] != 'B' ||
        header->magic[2] != 'L' || header->magic[3] != 'E') {
        print_string("Execution error: Invalid .eu custom binary header.\n");
        return;
    }

    // Process our custom bytecode stream manually
    uint8_t* bytecode = (uint8_t*)(f->data + sizeof(UbleHeader));
    uint16_t pc = 0; // Virtual Program Counter
    bool running = true;

    // Virtual Architecture Setup
    int32_t virtual_registers[8] = {0, 0, 0, 0, 0, 0, 0, 0}; 
    bool zero_flag = false;
    active_color = COLOR; // Reset color state to system standard before entry

    print_string("[Uble Virtual Environment Active]\n");
    ...
    while (running && pc < header->program_len && (sizeof(UbleHeader) + pc) < MAX_FILE_DATA) {
        ...
        switch (opcode) {
            ...
            case OP_PRINT_STR: {
                // Reads inline string until null-terminator
                while (bytecode[pc] != 0x00 && (sizeof(UbleHeader) + pc) < MAX_FILE_DATA) {
                    put_char((char)bytecode[pc++]);
                }
                pc++; // Skip past the null byte
                break;
            }
            case OP_LOAD_REG: {
            ...
            case OP_ADD: {
                ...
                if (regA < 8 && regB < 8) {
                    virtual_registers[regA] += virtual_registers[regB];
                }
                break;
            }
            case OP_SUB: {
            ...
            case OP_JZ: {
                ...
                uint16_t target = (bytecode[pc] << 8) | bytecode[pc + 1];
                pc += 2;
                if (reg < 8 && virtual_registers[reg] == 0) {
                    pc = target;
                }
                break;
            }
            ...
            case OP_CMP: {
                ...
                uint8_t regB = bytecode[pc++];
                if (regA < 8 && regB < 8) {
                    zero_flag = (virtual_registers[regA] == virtual_registers[regB]);
                }
                break;
            }
            case OP_INC: {
            ...
            default: {
                print_string("\nRuntime fault: Illegal opcode encountered.");
                running = false;
                break;
            }
        }
    }
    active_color = COLOR; // Force default layout cleanup
    print_string("\n[Uble Process Clean Exit]\n");
}
