...
#define MAX_FILE_DATA   16384  // Expanded to 16 KB per file entry
...
#define FS_MAX_SECTORS     16384  // Up to 8 MB of disk storage
...
void app_mem_diagnostics(void) {
    ...
    for(int i = 0; i < MAX_FILES; i++) {
        if(files[i].used) {
            used_files++;
            consumed_bytes += strlen(files[i].data);
        }
    }
        ...
        while (running && pc < header->program_len && (sizeof(UbleHeader) + pc) < MAX_FILE_DATA) {
            ...
            switch (opcode) {
                ...
                case OP_LOAD_REG: {
                    ...
                    if (reg < 8) { virtual_registers[reg] = val; }
                    break;
                }
                case OP_ADD: {
                ...
                case OP_JZ: {
                    ...
                    uint16_t target = (bytecode[pc] << 8) | bytecode[pc + 1];
                    pc += 2;
                    if (reg < 8 && virtual_registers[reg] == 0) { pc = target; }
                    break;
                }
                case OP_JNZ: {
                    uint8_t reg = bytecode[pc++];
                ...
                case OP_DEC: {
                    uint8_t reg = bytecode[pc++];
                    // FIXED: Typo successfully eradicated here
                    if (reg < 8) { virtual_registers[reg]--; }
                    break;
                }
                case OP_SET_COLOR: {
