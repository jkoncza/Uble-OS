...
void fs_delete_node_internal(
    ...
    int depth)
{
    if (idx < 0 || idx >= MAX_FILES || !files[idx].used)
        return;
    ...
    if (files[idx].is_dir)
    {
        ...
        for (int i = 0; i < MAX_FILES; i++)
        {
            ...
            if (strncmp(files[i].name, prefix, prefix_len) == 0 &&
                (files[i].name[prefix_len] == '/' || files[i].name[prefix_len] == '\0'))
            {
...
void process_command(const char* cmdline) {
    ...
    if (strcmp(cmd, "help") == 0) {
        ...
        draw_rect(TERM_X + 4, TERM_Y + 22,
                  TERM_WIDTH - 8, TERM_HEIGHT - 26,
                  PAL_BLACK);
...
void desktop_evaluate_click(int x, int y) {
    ...
    for (int i = num_widgets - 1; i >= 0; i--) {
        ...
        if (x >= widgets[i].x && x <= (widgets[i].x + widgets[i].w) &&
            ...
            if (widgets[i].action_id == 99) { start_menu_open = !start_menu_open; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 44) { terminal_open = false; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 55) { explorer_open = false; selected_file_idx = -1; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 66) { notepad_open = false; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 88) { settings_open = false; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 101) { explorer_open = true; start_menu_open = false; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 102) { notepad_open = true; start_menu_open = false; desktop_redraw_pipeline(); break; }
            if (widgets[i].action_id == 104) { terminal_open = true; start_menu_open = false; desktop_redraw_pipeline(); break; }
            ...
            if (widgets[i].action_id == 713)
            {
                if (selected_file_idx != -1 && strlen(prompt_text_buffer) > 0)
                {
                    ...
                    if (files[selected_file_idx].is_dir)
                    {
                        ...
                        /* ensure directory prefix is stable once */
                        char old_dir[128];
            ...
            if (widgets[i].action_id == 504) {
                if (strcmp(current_working_dir, "/desktop/") != 0 &&
                    strcmp(current_working_dir, "/") != 0) {

                    int len = strlen(current_working_dir);
                    int j = len - 2;

                    while (j >= 0 && current_working_dir[j] != '/')
                        j--;
            ...
            break; // stop widget loop after handling click
        }
    }
