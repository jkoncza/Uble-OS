        ...
        if (key != 0) {
            ...
            else if (terminal_open)
            {
                if (key == '\n')
                {
                    ...
                    // move cursor FIRST (so output prints correctly)
                    term_cursor_x = term_x + 5;
                    term_cursor_y += 12;
                    ...
                    // RESET input BEFORE execution (prevents stale buffer issues)
                    int saved_idx = cmd_idx;
                    char local_cmd[128];
                    ...
                    // IMPORTANT: execute command AFTER buffer is safe
