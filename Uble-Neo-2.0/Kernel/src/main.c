...
void kernel_main(void) {
    ...
    if (!e1000_init()) {
        ...
        * ============================================================
        *
        * QEMU's default user-mode network normally uses:
        *
        ...
        *   "Who has 10.0.2.2?"
        *
        * The gateway should answer with its MAC address.
        */
        ...
        ipv4_address_t ipv4_local_ip = {{ 10, 0, 2, 15 }};

        ipv4_init(ipv4_local_ip);
        ...
        if (udp_bind(12345)) {
            print_string("UDP: listening on port 12345.\n");
        } else {
            print_string("UDP: failed to bind port 12345.\n");
        }
        if (!arp_send_request(gateway_ip)) {
            print_string("ARP: request failed to send.\n");
        }
        else
        {
            print_string(
            ...
            * Later we'll replace this with proper networking
