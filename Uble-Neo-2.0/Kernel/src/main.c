...
void cmd_help(void) {
    ...
    print_string("  ping <IPv4>             - Send an ICMP Echo Request\n");
...
void cmd_ping(const char *rest) {
    char ip_text[32];
    const char *dummy;
    split_first(rest,ip_text,sizeof(ip_text),&dummy);
    if (!*ip_text) {
        print_string("Usage: ping <IPv4 address>\n");
        return;
    }
    /*
     * For now, support the four decimal IPv4
     * components directly.
     *
     * Example:
     *
    ...
    while (*p)
    {
        if (*p >= '0' && *p <= '9') {
            value =
                ...
                (uint32_t)(*p - '0');
            if (value > 255) {
                print_string("ping: invalid IPv4 address.\n");
                return;
            }
        } else if (*p == '.') {
            if (component == 0)
    ...
    print_string("PING ");
    ipv4_print_address(destination);
    print_string("\n");
    /*
     * Make sure ARP knows the destination MAC.
     */
    ...
    if (!arp_lookup(
        ...
        destination_mac)) {
        print_string("ping: ARP cache miss.\n");
        print_string("ping: sending ARP request...\n");
        ...
        if (!arp_send_request(arp_destination)) {
            print_string("ping: ARP request failed.\n");
            return;
        }
        /*
         * Wait for ARP reply.
         */
        bool arp_received = false;
        ...
        if (!arp_received) {
            print_string("ping: ARP timeout.\n");
            return;
        }
        if (!arp_lookup(arp_destination,destination_mac)) {
            print_string("ping: ARP lookup failed.\n");
            return;
        }
    }
    /*
     ...
     */
    if (!icmp_send_echo_request(destination,0x1234,1)) {
        print_string("ping: Echo Request failed.\n");
        return;
    }
    print_string("ping: Echo Request sent.\n");
    print_string("ping: waiting for reply...\n");
    /*
     * Poll IPv4/ICMP.
     */
    for (uint32_t timeout = 0; timeout < 5000000; timeout++) {
        if (ipv4_poll()) {
            print_string("ping: reply received.\n");
            return;
        }
    }
    print_string("ping: timeout.\n");
}
...
void kernel_main(void)
{
    ...
    print_string(
        " Systems Identity Initialized. Default: admin / admin\n"
    );

    print_string(
        " Type 'help' to dump shell system instruction map\n\n"
    );
     ...
     * ============================================================
     *
     * E1000 is the hardware/network-card layer.
     */
    ...
    else
    {
         ...
         * ========================================================
         *
         * Ethernet sits directly above E1000.
         *
         * IMPORTANT:
         *
         * E1000 MUST be initialized before Ethernet.
         */
        ...
        else
        {
             ...
             *     arp_process_frame()
             *
             * ARP itself no longer directly owns the E1000 RX
             * interface.
             */
            ...
            )
            {
                ...
                )
                {
                    ...
                    ipv4_address_t udp_test_destination =
                    {
                        {
                            10,
                            0,
                    ...
                    {
                        print_string(
                            "UDP: test datagram submitted.\n"
                        );
                    }
                    else
                    {
                        print_string(
                            "UDP: test datagram FAILED.\n"
                        );
                    }
                     ...
                     * NOTE:
                     *
                     * This still uses the old UDP polling path.
                     * We will refactor UDP/IPv4 next so that it also
                     * receives frames through Ethernet.
                     */
                    ...
                    ipv4_address_t ipv4_gateway_ip =
                    {
                        {
                            gateway_ip.bytes[0],
                            gateway_ip.bytes[1],
                    ...
                    {
                        print_string(
                            "ICMP: ping request submitted.\n"
                        );
                    }
                    else
                    {
                        print_string(
                            "ICMP: ping request FAILED.\n"
                        );
                    }


                    print_string(
                        "ICMP: waiting for Echo Reply...\n"
                    );
