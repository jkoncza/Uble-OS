...
bool save_usf(
    ...
    char *source)
{
    uint8_t program[MAX_FILE_DATA];
    uint32_t len =
        assemble_text(source,program);
    uint8_t file[MAX_FILE_DATA];
    UbleHeader *hdr =
        (UbleHeader*)file;
    hdr->magic[0]='U';
    hdr->magic[1]='B';
    ...
    uint8_t *program)
{
    ...
    while(*line)
    {
        ...
        size +=
            assemble_line(tokens,count,&program[size]);
    }
    return size;
}
    ...
    if (raw_bytes[0] == 'U' && raw_bytes[1] == 'B' && raw_bytes[2] == 'L' && raw_bytes[3] == 'E') {
        ...
        while (running && pc < header->program_len && (sizeof(UbleHeader) + pc) < MAX_FILE_DATA) {
            ...
            switch (opcode) {
                ...
                case OP_STORE_MEM: {
                    uint8_t reg = bytecode[pc++];
                    uint16_t addr = bytecode[pc++];
                    if(reg < 8 && addr < 4096)
                        virtual_memory[addr]=virtual_registers[reg];
                    break;
                }
                case OP_LOAD_MEM: {
                    uint8_t reg = bytecode[pc++];
